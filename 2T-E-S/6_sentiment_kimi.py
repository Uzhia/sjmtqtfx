...
# 用于记录已处理的评论 ID
processed_comments_file_name = "processed_comments_kimi2.csv"
...
def fetch_comments_with_weibo_kimi(conn, processed_ids):
    try:
...
""".strip()
        ...
        if data["error"].find("max request") != -1:
            return None
        if data["error"].find("Rate limit exceeded") != -1:
            return None
        # 将 data 写入文件
...
def main():
    # 在4_sentiment.py中值为无法得到结果的的评论(bad request)中，再通过kimi处理一下
    file_name = "6_sentiment_results_kimi.csv"
    ...
    data = fetch_comments_with_weibo_kimi(conn, processed_ids)
